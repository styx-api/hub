	...
	function createPythonStaticCall(
		...
		function buildParams(
			...
			for (const [key, value] of Object.entries(config)) {
				...
				const fnStruct = propSpec.fn_struct_make_params;

				if (fnStruct && typeof value === 'object' && value !== null && !Array.isArray(value)) {
			...
			<Card>
				<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-3">
					<div>
						...
						<CardDescription>
							{isMobile ? 'Current parameter values in JSON format' : 'Parameter values in JSON'}
						</CardDescription>
					</div>
					{#if !isMobile}
						<Button
