...
{#snippet renderTree(node: TreeNode, name: string, path: string)}
	...
	<div>
		<div
			class={cn(
				'group flex cursor-pointer items-center rounded-md px-3 py-2 transition-colors',
		...
		>
			<div class="flex min-w-0 flex-1 items-center">
				{#if hasChildren}
					<button
						class={cn('mr-2 rounded p-0.5 transition-colors', 'hover:bg-accent/50')}
				...
				{#if node.type === 'folder'}
					<FolderOpen class="mr-3 h-4 w-4 flex-shrink-0 text-primary" />
				{:else}
					<FileText class="mr-3 h-4 w-4 flex-shrink-0 text-muted-foreground" />
				{/if}
					...
					{#if node.data}
						<div class="flex items-center justify-between gap-2">
							<div class="min-w-0 flex-1">
								...
								<div class="mt-0.5 line-clamp-2 text-xs text-muted-foreground/70">
									{node.data.description ?? 'Description'}
								</div>
							</div>
							<button
								class={cn(
									'flex-shrink-0 rounded-md border px-2 py-1 font-mono text-xs transition-all duration-150',
