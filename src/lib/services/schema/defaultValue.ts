...
import { type JSONSchema, isSchemaObject, isObjectSchema, isArraySchema, isStringSchema, isNumberSchema, isIntegerSchema, isBooleanSchema, isNullSchema } from "./schema";
...
function generateNumber(schema: JSONSchema.Number | JSONSchema.Integer, isInteger: boolean): number {
  const min = schema.minimum ?? (schema.exclusiveMinimum !== undefined ? schema.exclusiveMinimum + (isInteger ? 1 : 0.01) : 0);
  const max = schema.maximum ?? (schema.exclusiveMaximum !== undefined ? schema.exclusiveMaximum - (isInteger ? 1 : 0.01) : undefined);
