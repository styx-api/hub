...
export async function getPackage(projectName: string, packageName: string): Promise<PackageType> {
...
export async function* iterProjects(projects: ProjectsType): AsyncGenerator<ProjectType> {
	for (const name of projects.projects) {
		yield await getProject(name);
	}
}

export async function* iterPackages(project: ProjectType): AsyncGenerator<PackageType> {
...
export async function* iterAllPackages(projects: ProjectsType): AsyncGenerator<PackageContext> {
...
export async function* iterAllVersions(projects: ProjectsType): AsyncGenerator<VersionContext> {
	for await (const project of iterProjects(projects)) {
		const projectName = project.name;
		for await (const pkg of iterPackages(project)) {
...
export async function* iterAllApps(projects: ProjectsType): AsyncGenerator<AppContext> {
	for await (const project of iterProjects(projects)) {
		const projectName = project.name;
		for await (const pkg of iterPackages(project)) {
			const packageName = pkg.name;
